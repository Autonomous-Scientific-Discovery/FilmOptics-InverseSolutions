# -*- coding: utf-8 -*-
"""
Created on Thu Jan 21 12:54:49 2021
...
# =============================================================================
# Modified totLossRT_tl to have return_RT option
# =============================================================================
def totLossRT_tl(decn_var, return_RT=False):
...
#        n_layer1_new=np.zeros(len(lam))
#        k_layer1_new=np.zeros(len(lam))
...
opts.set('bounds', [np.hstack([np.repeat([0,0,0],numTLO),np.array([0,0,20])]), 
...
for i in range(nsamples*nruns):
    ...
    res_CMAES=cma.fmin2(totLossRT_tl, x_startCMA, sigma_start, opts) # restarts=8, bipop=True)
#    rec_cma.append([indxRef, data_perv['y'][indxRef,-1], res_CMAES[0][-1], res_CMAES[1].result[1], res_CMAES[1].result[4], res_CMAES[1].timer.elapsed, res_CMAES[1].opts['seed']])   # y_train[2][indxRef,0] 
    # =============================================================================
    # Changed and added code
    # =============================================================================
    # n, k =  y_train[0][indxRef], y_train[1][indxRef]    
    n, k = data_perv['y'][indxRef,0:651], data_perv['y'][indxRef,651:1302]     # 
    n_hat, k_hat = nk_VensembleTL(lam,res_CMAES[0][:-1])
    # R, T = X_train[indxRef, :, 0], X_train[indxRef, :, 1]   
    R, T = data_perv['X'][indxRef,:,0], data_perv['X'][indxRef,:,1]    # 
    R_hat, T_hat = totLossRT_tl(res_CMAES[0], return_RT=True)
    ...
    if np.abs(d-d_hat)< 0.1*d and error_hat< 0.17:
        ...
        pickle.dump(result, open('resultB_TLO.pickle', 'wb'))
    
# =============================================================================
# Read the pickle file 
# =============================================================================
result = pickle.load(open('resultB_TLO.pickle', 'rb'))

# =============================================================================
# Obatin the metrics for thickness
# =============================================================================
...
print(f'success rate is {len(r2_T)/100}')   